...
namespace com.IvanMurzak.Godot.MCP.PhantomCamera.Tests
{
    ...
    /// PURE-MANAGED sample tool <c>phantomcamera-echo</c> by constructing the tool family and
    ...
    /// </summary>
    public class Tool_PhantomCamera_EchoTests
    {
        [Fact]
        ...
        {
            var tool = new Tool_PhantomCamera();
            Assert.Equal("phantomcamera-ready", tool.Echo(message));
        }
        ...
        public void EchoToolId_IsStable()
        {
            ...
            Assert.Equal("phantomcamera-echo", Tool_PhantomCamera.EchoToolId);
        }
    }
}
